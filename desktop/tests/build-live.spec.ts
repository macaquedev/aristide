...
// ARISTIDE_LIVE=1 bunx playwright test build-live
...
test('Build edits a stop rule live without interrupting a held note', async ({ page }) => {
  await page.goto('/');
  ...
  // Build opens through the stop, in edit mode only.
  ...
  await page.screenshot({ path: 'test-results/build-live.png', fullPage: true });

  // Global undo steps back through Build's edits.
  ...
  await page.getByRole('button', { name: 'Build', exact: true }).click();
  await expect(page.locator('.roll-stop-name')).toContainText(stop.name);
  await page.screenshot({ path: 'test-results/build-live-phone.png', fullPage: true });
  ...
  // Locking returns to Play and closes Build.
