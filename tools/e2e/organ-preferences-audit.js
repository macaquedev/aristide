...
try {
  ...
  const open=async()=>{await d.click('#instrument-settings');await sleep(150);};
  const category=async id=>{await d.set('#organ-prefs-section',id);await sleep(100);};
  ...
  check(await d.eval(`[...document.querySelectorAll('.editor-toolbar button')].every(b=>b.offsetHeight>=44)`),'editing buttons have at least 44px touch targets');
  await tap('#editor-add-button');check(await visible('#editor-add-menu'),'Add opens with an actual finger tap');
  await tap('#organ-prefs .modal-close');await tap('#instrument-settings');check(await visible('#organ-prefs'),'Settings opens centralized preferences with an actual finger tap');
  ...
  await category('stops');await d.eval(`document.querySelector('#organ-prefs-index .organ-pref-action').click()`);await sleep(150);
  ...
  await category('couplers');await d.eval(`document.querySelectorAll('#organ-prefs-index .organ-pref-action')[1].click()`);await sleep(150);
  check(await visible('#editor-coupler'),'coupler routes are editable in preferences');
  check(await d.eval(`document.querySelector('#prefs [data-pane="appearance"]')!==null && !document.querySelector('#organ-prefs [data-pane="appearance"]')`),'appearance has one home in App preferences');
  check(await d.eval(`(()=>{const r=document.querySelector('#organ-prefs .modal-card').getBoundingClientRect();return r.left>=0&&r.right<=innerWidth&&r.bottom<=innerHeight;})()`),'settings stays within the phone viewport');
  await category('sources');await sleep(200);check(await d.eval(`document.querySelector('#organ-prefs-index').textContent.includes('Sample set tuning')`),'sample set tuning is available in preferences');
  ...
  await tap('#tuning');check(await visible('#editor-tuning')&&await visible('#organ-prefs'),'the tuning shortcut opens the same settings workspace');
  await d.send('Input.dispatchKeyEvent',{type:'keyDown',key:'Escape',code:'Escape'});
  await open();await category('general');
  await category('keyboards');await action('Add keyboard');
  ...
  await category('general');await action('Save a copy');
  ...
  const twin = (await state()).couplers.find(c=>!c.hidden);
  await post(`/api/organ/coupler/add?name=Duplicate%20routes&routes=${encodeURIComponent(JSON.stringify(twin.routes))}`);
  await settled(); await sleep(300);
  await category('couplers'); await action('Duplicate routes');
  await tap('#organ-prefs .modal-close');
  check(await visible('#editor-link-confirm')&&await visible('#organ-prefs'),'closing a duplicate coupler preserves the existing link choice');
  await tap('#editor-link-confirm-no');
  await tap('#organ-prefs .modal-close');
  check(!(await visible('#organ-prefs')),'the workspace closes after the link choice is answered');
