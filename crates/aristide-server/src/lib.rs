...
use audio::{AudioOutput, pick_f32_config, spawn_recorder};
...
pub fn run_cli() -> Result<()> {
    ...
    run_server(parse_args()?, None)
}

/// The device-owning loop. Desktop hosts run this on a dedicated thread;
/// changing a webview or panel never owns, drops or restarts its stream.
fn run_server(args: Args, ready: Option<desktop::Ready>) -> Result<()> {
    tracing::info!(
...
fn adopt_set(state: &Mutex<State>, request: LoadRequest, progress: &dyn Fn(String)) -> LoadRequest {
    ...
    if let Some(path) = wrapper {
        tracing::info!("organ file for {}: {}", set.display(), path.display());
        let mut state = state.lock().expect("state poisoned");
    ...
    let name = match aristide_formats::sidecar::load_for(&set) {
        Ok(Some(sidecar)) if !sidecar.name.trim().is_empty() => sidecar.name.trim().to_string(),
        _ => organ.name.clone(),
    };
    let wiring = state
        .lock()
